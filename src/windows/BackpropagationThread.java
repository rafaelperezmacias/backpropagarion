...
public class BackpropagationThread extends Regression implements Runnable {
    ...
    private BackpropagationWindow window;
    private DataSet dataSet;
    private AbstractParams params;

    public BackpropagationThread()
    {

    }

    public void makeModel(DataSet dataSet, AbstractParams params, BackpropagationWindow window) throws Exception {
    ...
    public void run() {
        ...
        try {
            X = DataSetFunctions.generateNumericMatrix(dataSet, getColumns(dataSet, idxTarget));
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        ...
        while ( error > minError && epoch < epochs ) {
            ...
            epoch++;
            window.updateErrorAndEpoch(false, epoch < epochs, error, epoch, learningRate);
            window.addErrorForChart(epoch, error, false);
        }
        // Modelo obtenido
        window.addErrorForChart(epoch, error, true);
        window.updateErrorAndEpoch(true, epoch <= epochs && error < minError, error, epoch, learningRate);
        window.setModel(new Model(dataSet, layers, targets, minError, error, epochs, epoch));
    }
    ...
    private void setupLayers(ArrayList<Layer> layers, DataSet dataSet, Params params, HashMap<String, double[]> targets) {
        // First layer
        Layer firstLayer = new Layer(dataSet.getHeaders().size() - 1, 0);
        setupFirstLayer(firstLayer);
    ...
    private void setupFirstLayer(Layer layer) {
        layer.outputs = new double[layer.neurons + 1];
        layer.outputs[0] = -1;
    }
    ...
    private static void validateDataSetAndParams(DataSet dataSet, AbstractParams params) throws Exception {
        ...
        if ( !(params instanceof BackpropagationThread.Params) ) {
